...
impl SendFilesBubble {
    ...
    pub async fn cancel(&self) -> Result<()> {
        self.handler.log("cancel: Initiating file transfer cancellation".to_string());
        ...
        match &result {
            Ok(_) => {
                self.handler.log("cancel: File transfer cancelled successfully".to_string());
            }
            Err(e) => {
                self.handler.log(format!("cancel: Error during cancellation: {}", e));
            }
        }
    ...
    pub fn is_finished(&self) -> bool {
        ...
        if is_finished {
            self.handler.log("is_finished: Transfer is finished, ensuring router shutdown".to_string());
    ...
    pub fn is_connected(&self) -> bool {
        ...
        if finished {
            self.handler.log("is_connected: Transfer is finished, returning false".to_string());
            return false;
        }
        
        let consumed = self.handler.is_consumed();
        self.handler.log(format!("is_connected: Handler consumed: {}", consumed));
    ...
    pub fn subscribe(&self, subscriber: Arc<dyn SendFilesSubscriber>) {
        ...
        self.handler.log(format!("subscribe: Subscribing new subscriber with ID: {}", subscriber_id));
        self.handler.subscribe(subscriber);
    }

    pub fn unsubscribe(&self, subscriber: Arc<dyn SendFilesSubscriber>) {
        let subscriber_id = subscriber.get_id();
        self.handler.log(format!("unsubscribe: Unsubscribing subscriber with ID: {}", subscriber_id));
...
pub async fn send_files(request: SendFilesRequest) -> Result<SendFilesBubble> {
    ...
    handler.log(format!("send_files: Starting file transfer initialization with {} files", files_len));
    handler.log(format!("send_files: Buffer size configuration: {} bytes", request.config.buffer_size));

    handler.log("send_files: Creating endpoint builder with discovery_n0".to_string());
