...
/// Helper to run arkdrop-cli commands
...
impl ARKDropRunner {
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        Ok(Self {
            binary_path: PathBuf::from("target/release/arkdrop-cli"),
        })
    }
...
mod tests {
    ...
    fn test_basic_file_transfer() {
        ...
        let (mut receiver, ticket, code) = runner
            .start_receiver("Test Receiver", &out_dir)
        ...
        let received_file = out_dir.join("test.txt");
    ...
    fn test_multiple_file_transfer() {
        ...
        // Start receiver
        let (mut receiver, ticket, code) = runner
            .start_receiver("Multi Receiver", &out_dir)
            .expect("Failed to start receiver");
        ...
        for i in 0..3 {
            let received_file = out_dir.join(format!("test{i}.txt"));
