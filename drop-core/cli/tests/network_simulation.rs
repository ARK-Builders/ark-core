...
impl NetworkSimulator {
    ...
    pub fn add_packet_loss(&mut self, percentage: f32) -> Result<(), Box<dyn std::error::Error>> {
        self.save_state()?;
        
        Command::new("sudo")
            .args(&["tc", "qdisc", "add", "dev", &self.interface, "root", "netem", "loss", &format!("{}%", percentage)])
            .output()?;
    ...
    pub fn add_latency(&mut self, delay_ms: u32) -> Result<(), Box<dyn std::error::Error>> {
        self.save_state()?;
        
        Command::new("sudo")
            .args(&["tc", "qdisc", "add", "dev", &self.interface, "root", "netem", "delay", &format!("{}ms", delay_ms)])
            .output()?;
    ...
    pub fn limit_bandwidth(&mut self, rate_kbps: u32) -> Result<(), Box<dyn std::error::Error>> {
        self.save_state()?;
    ...
    pub fn add_jitter(&mut self, base_delay_ms: u32, jitter_ms: u32) -> Result<(), Box<dyn std::error::Error>> {
        self.save_state()?;
        
        Command::new("sudo")
            .args(&[
                "tc", "qdisc", "add", "dev", &self.interface, "root", "netem",
                "delay", &format!("{}ms", base_delay_ms), &format!("{}ms", jitter_ms),
                "distribution", "normal"
            ])
            .output()?;
    ...
    fn save_state(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.original_state = Some(String::from_utf8_lossy(&output.stdout).to_string());
        Ok(())
    }
...
mod tests {
    ...
    fn test_packet_loss_simulation() {
        ...
        simulator.add_packet_loss(5.0).expect("Failed to add packet loss");
        
        // Run transfer test here
        
        simulator.reset().expect("Failed to reset network");
    }
    ...
    fn test_latency_simulation() {
        let mut simulator = NetworkSimulator::new("lo");
        simulator.add_latency(100).expect("Failed to add latency");
    ...
    fn test_basic_file_transfer() {
        ...
        let content = fs::read_to_string(&received_file).expect("Failed to read received file");
    ...
    fn test_multiple_file_transfer() {
        ...
        for i in 0..3 {
            ...
            let content = fs::read_to_string(&received_file).expect("Failed to read received file");
            assert_eq!(content, format!("Content {}", i), "File {} content mismatch", i);
        }
